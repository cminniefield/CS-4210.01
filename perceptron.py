...
# SPECIFICATION: Train and evaluate both a single-layer perceptron and a multi-layer perceptron on the optdigits dataset using various hyperparameters.
...
for rate in n: #iterates over n
    ...
    for shuffle in r: #iterates over r
        ...
        for model in ['Perceptron', 'MLP']: #iterates over the algorithms
            ...
            if model == 'Perceptron':
                clf = Perceptron(eta0=rate, shuffle=shuffle, max_iter=1000, random_state=42)
            else:
                clf = MLPClassifier(activation='logistic', learning_rate_init=rate,
                                    hidden_layer_sizes=(25,), shuffle=shuffle, max_iter=1000, random_state=42)
            ...
            if model == 'Perceptron' and accuracy > highest_accuracy_perceptron:
                highest_accuracy_perceptron = accuracy
                print(f"Highest Perceptron accuracy so far: {accuracy:.2f}, Parameters: learning rate={rate}, shuffle={shuffle}")

            if model == 'MLP' and accuracy > highest_accuracy_mlp:
                highest_accuracy_mlp = accuracy
                print(f"Highest MLP accuracy so far: {accuracy:.2f}, Parameters: learning rate={rate}, shuffle={shuffle}")
